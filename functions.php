...
/* ============================================================
   Localization (Bangla / English interface)
   ============================================================ */

// UI language: session-stored, switchable via ?lang=bn / ?lang=en
if (session_status() === PHP_SESSION_NONE && !headers_sent()) { @session_start(); }
$__uiLang = isset($_GET['lang']) ? $_GET['lang'] : '';
if (in_array($__uiLang, array('bn', 'en'), true)) {
    $_SESSION['ui_lang'] = $__uiLang;
}
$GLOBALS['UI_LANG'] = isset($_SESSION['ui_lang']) && $_SESSION['ui_lang'] === 'bn' ? 'bn' : 'en';
unset($__uiLang);

// Translation dictionary. English strings are used as-is.
function t($key) {
    static $dict = array(
        'bn' => array(
            'Home' => 'হোম',
            ...
            'DMCA / Copyright' => 'ডিএমসিএ / কপিরাইট',
            'Disclaimer' => 'দাবিত্যাগ',
            'Continue Watching' => 'দেখা চালিয়ে যান',
            'Featured Movies' => 'ফিচার্ড মুভি',
            'Trending Now' => 'এখন ট্রেন্ডিং',
            'Recent Movies' => 'সাম্প্রতিক মুভি',
            'Latest Anime' => 'সর্বশেষ অ্যানিমে',
            'Popular in Bangladesh' => 'বাংলাদেশে জনপ্রিয়',
            'View All' => 'সব দেখুন',
            'Watch Now' => 'এখনই দেখুন',
            'Browse Movies' => 'মুভি ব্রাউজ করুন',
            'No Movies Found' => 'কোনো মুভি পাওয়া যায়নি',
            'No Anime Found' => 'কোনো অ্যানিমে পাওয়া যায়নি',
            'Report Broken Video' => 'ভাঙা ভিডিও রিপোর্ট করুন',
            'Request a Title' => 'টাইটেল রিকোয়েস্ট করুন',
            'Related Movies' => 'সম্পর্কিত মুভি',
            'Episodes' => 'এপিসোড',
        ),
    );
    $lang = isset($GLOBALS['UI_LANG']) ? $GLOBALS['UI_LANG'] : 'en';
    if ($lang === 'en') { return $key; }
    return isset($dict[$lang][$key]) ? $dict[$lang][$key] : $key;
}

/** Current UI language code ('en' or 'bn'). */
function uiLang() {
    return isset($GLOBALS['UI_LANG']) ? $GLOBALS['UI_LANG'] : 'en';
}

/** URL to switch the UI language, preserving the current path. */
function langSwitchUrl($target) {
    $uri = isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '/';
    $parts = parse_url($uri);
    $query = array();
...
/** True when the visitor asked for low-data mode (cookie or ?data=low). */
function isLowDataMode() {
    if (isset($_GET['data']) && $_GET['data'] === 'low') { return true; }
    if (isset($_GET['data']) && $_GET['data'] === 'full') { return false; }
    return isset($_COOKIE['bdmh_lowdata']) && $_COOKIE['bdmh_lowdata'] === '1';
}
